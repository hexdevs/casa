...
$(() => {
  ...
  const formatOccurredAtDate = (record) =>
    new Date(record.occurred_at.replaceAll("-", " ")).toDateString();

  const mapContactTypes = (contactTypes) => {
    return mapValues(groupBy(contactTypes, "group_name"), (contactType) =>
      map(contactType, "name").join(", ")
    );
  };
  ...
  $('[data-filter="volunteer"] .select2').on("select2:select select2:unselect", () =>
    reimbursementsTable.draw()
  );
