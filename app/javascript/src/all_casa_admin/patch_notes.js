...
patchNoteFunctions.addPatchNoteUI = function (
  patchNoteGroupId,
  patchNoteId,
  patchNoteList,
  patchNoteText,
  ...
  const newPatchNoteUIFormInputs = patchNoteFunctions.getPatchNoteFormInputs(
    newPatchNoteUI.children()
  );
  ...
  newPatchNoteUIFormInputs.dropdownGroup
    .children(`option[value="${patchNoteGroupId}"]`)
    .attr("selected", true);
  newPatchNoteUIFormInputs.dropdownType.children().removeAttr("selected");
  newPatchNoteUIFormInputs.dropdownType
    .children(`option[value="${patchNoteTypeId}"]`)
    .attr("selected", true);
...
patchNoteFunctions.onCreate = function () {
  try {
    ...
    patchNoteFunctions
      .createPatchNote(patchNoteGroupId, patchNoteText, patchNoteTypeId)
      .then(function (response) {
        newPatchNoteFormInputs.noteTextArea.val("");
        patchNoteFunctions.addPatchNoteUI(
          patchNoteGroupId,
          response.id,
          patchNoteList,
          patchNoteText,
...
patchNoteFunctions.onDeletePatchNote = function () {
  ...
  switch (deleteButton.text().trim()) {
    ...
    case "1":
      ...
      patchNoteFunctions
        .deletePatchNote(patchNoteFunctions.getPatchNoteId(patchNoteFormContainer))
        .then(function () {
...
patchNoteFunctions.onSavePatchNote = function () {
  ...
  patchNoteFunctions
    .savePatchNote(patchNoteGroupId, patchNoteId, patchNoteText, patchNoteTypeId)
    .then(function (response) {
      patchNoteFormInputs.noteTextArea.prop("disabled", true);
      patchNoteFormInputs.dropdownGroup.prop("disabled", true);
      patchNoteFormInputs.dropdownType.prop("disabled", true);
...
patchNoteFunctions.patchNoteFormDataResetBeforeEdit = function (patchNoteFormInputs) {
  ...
  try {
    patchNoteDataBeforeEditing =
      patchNoteFormBeforeEditData[
        patchNoteFunctions.getPatchNoteId(patchNoteFormInputs.noteTextArea.parent())
      ];
...
//  @throws   {TypeError}  for a parameter of the incorrect type
//  @throws   {RangeError} if an id parameter is negative
patchNoteFunctions.savePatchNote = function (
  patchNoteGroupId,
  patchNoteId,
  patchNoteText,
  patchNoteTypeId
